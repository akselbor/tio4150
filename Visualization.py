...
def add_edge_to_df(df_cities, df_edges, from_city, to_city):
    ...
    df_edges = df_edges.append({'start_lat':start_lat, 'start_lon':start_lon, 'end_lat':end_lat, 'end_lon':end_lon}, ignore_index=True)
    return df_edges

def plot_sol(sol, number_of_cities):
    ...
    df_edges = pd.DataFrame(columns=['start_lat', 'start_lon', 'end_lat', 'end_lon'])
    ...
    for i in range(len(df_edges)):
        ...
        hover_data = df_sol['From'][i] + ' - ' + df_sol['To'] + ': ' + str(round(flow,2))
